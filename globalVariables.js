...
async function getCategories() {
  try {
    let responseServer = await fetch(
      "https://jonas34.pythonanywhere.com/categories/",
      { method: "GET", headers: { "Content-Type": "application/json" } }
    );
    if (!responseServer.ok) throw new Error("Response not ok");
...
async function getSubtasks() {
  try {
    let responseServer = await fetch(
      "https://jonas34.pythonanywhere.com/subtasks/",
      { method: "GET", headers: { "Content-Type": "application/json" } }
    );
    if (!responseServer.ok) throw new Error("Response not ok");
...
async function getTodos() {
  try {
    let responseServer = await fetch(
      "https://jonas34.pythonanywhere.com/todos/",
      { method: "GET", headers: { "Content-Type": "application/json" } }
    );
    if (!responseServer.ok) throw new Error("Response not ok");
