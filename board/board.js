...
function updateHTMLOpenTasks() {
  ...
  for (let index = 0; index < openTasks.length; index++) {
    ...
    document.getElementById("1").innerHTML += generateTodoHTML(
      element,
      openIndex
    );
...
function updateHTMLInProgessTasks() {
  ...
  for (let index = 0; index < progress.length; index++) {
    ...
    document.getElementById("2").innerHTML += generateTodoHTML(
      element,
      progressIndex
    );
...
function updateHTMLFeedbackTasks() {
  ...
  for (let index = 0; index < feedback.length; index++) {
    ...
    document.getElementById("3").innerHTML += generateTodoHTML(
      element,
      feedbackIndex
    );
...
function updateHTMLClosedTasks() {
  ...
  for (let index = 0; index < closed.length; index++) {
    ...
    document.getElementById("4").innerHTML += generateTodoHTML(
      element,
      closedIndex
    );
...
function getSubtask(task, index) {
  ...
  if (TaskTotal > 0) {
    document.getElementById("boxSubTask" + index).innerHTML = generateSubtasks(
      subtaskTrue.length,
      TaskTotal,
      index
    );
    let taskPercentDone = (subtaskTrue.length / TaskTotal) * 100;
    document.getElementById("subtaskBar" + index).style.width =
      taskPercentDone + "%";
  }
}
...
function getPrio(task, index) {
  if (task.priority === "H") {
    document.getElementById("prio" + index).src =
      "../assets/img/urgent_task.png";
  } else if (task.priority === "M") {
    document.getElementById("prio" + index).src =
      "../assets/img/medium_task.png";
...
function addUserToBoard(index, splitUser, i, task, letters) {
  ...
  document.getElementById(
    "checkUsers" + index
  ).innerHTML += `<div class="todo_contact_img" id="checkUser${index}${i}">${letters[0]}${letters[1]}</div>`;

  placeUser = 1.75 * i;
  if (i > 0) {
    document.getElementById("checkUser" + index + i).style.left =
      placeUser + "rem";
  }
  let filteredcontacts = contacts.filter((contact) => {
    if (splitUser.includes(contact.lastName)) return contact;
  });
  document.getElementById("checkUser" + index + i).style.backgroundColor =
    filteredcontacts[0].color;
  document.getElementById("checkCategory" + index).style.backgroundColor =
    task.categories[0].color;
}
...
function highlight(id) {
  ...
  if (filteredTask["status"] !== id) {
    ...
    let category = document
      .getElementById(id)
      .querySelector(".drag_area_highlight");
    if (category == null) {
...
function setSubtasks(fitleredTask, index) {
  for (let i = 0; i < fitleredTask["subtasks"].length; i++) {
    ...
    document.getElementById("place_subtasks").innerHTML +=
      generateSubtaskDetails(fitleredTask, i, index);
  }
  for (let i = 0; i < fitleredTask["subtasks"].length; i++) {
    const subtask = fitleredTask["subtasks"][i];
    if (subtask["done"] == "true") {
...
function checkDoneSubtasks(filteredTask, index) {
  ...
  document.getElementById("subtaskDone" + index).innerHTML =
    finishedSubtasks.length;
  document.getElementById("subtaskBar" + index).style.width =
    finishedPercentage + "%";
}
...
function addUserToDetails(id, letters, fitleredTask) {
  for (let i = 0; i < arraySplitUser[0].length; i++) {
    ...
    document.getElementById(
      id
    ).innerHTML += `<div class="assignedContact"> <div style="background-color:${filteredContact[0]["color"]}" class="details_contact_img">${letters[i]}</div> <p> ${element} </div>`;
  }
  arraySplitUser = [];
}
...
function setPriority(fitleredTask) {
  ...
  } else if (fitleredTask["priority"] == "L") {
    // Sets color for selected task depending on priority
    priorityColor = "green";
    priorityDetails = "Low";
...
function setCurrentStatus(fitleredTask) {
  if (fitleredTask["status"] == "1") {
    currentStatus = "To do";
  } else if (fitleredTask["status"] == "2") {
    // Sets current status for selected task
...
function changeUrgency(prio, id) {
  ...
  if (prio === "H") {
    document
      .getElementById("changeUrgentButton")
      .classList.toggle("box_button_task_u");
    document
      .getElementById("changePrioUrgent")
      .classList.toggle("urgency_img_u_clicked_task");
    document
      .getElementById("changePrioUrgent")
      .classList.toggle("urgency_img_u_task");
  } else if (prio === "L") {
    document
      .getElementById("changeLowButton")
      .classList.toggle("box_button_task_l");
    document
      .getElementById("changePrioLow")
      .classList.toggle("urgency_img_l_clicked_task");
    document
      .getElementById("changePrioLow")
      .classList.toggle("urgency_img_l_task");
  } else if (prio === "M") {
    document
      .getElementById("changeMediumButton")
      .classList.toggle("box_button_task_m");
    document
      .getElementById("changePrioMedium")
      .classList.toggle("urgency_img_m_clicked_task");
    document
      .getElementById("changePrioMedium")
      .classList.toggle("urgency_img_m_task");
  }
}
